  ...
  <div>
    <div class="placeOrder" v-if="roomTypeList.length > 0">
      <div class="nav">
        ...
        >
          <span
            :class="(index == currentTypeIndex)?'greenFont':''"
          >{{type}}{{roomList[index].roomId}}</span>
...
export default {
  ...
  methods: {
    ...
    getSeatList() {
      ...
      const url = `/customer/sits/${this.storeId}/${
        this.roomList[this.currentTypeIndex].roomId
          ...
          if (res.code != 0) {
            ...
            if (sitList.length > 0) {
              ...
              sitList.forEach(sit => {
                for (let i = 0; i < this.seatList.length; i++) {
                  ...
                  if (seat.sitId == sit.sitId) {
                    if (
                      seat.curDate[0] &&
                      seat.curDate[0].value == sit.curDate
                    ) {
                      ...
                      seat.curDate[1] &&
                      seat.curDate[1].value == sit.curDate
                    ) {
                      if (sit.preserved != 1) {
                        //如果作为没有被预定，则把时间段加进去
                  ...
                  if (
                    i == this.seatList.length - 1 &&
                    seat.sitId != sit.sitId
                  ) {
                    let tseat = {};
                    tseat.curDate = [];
                    let tempCurDate = {};
              ...
              if (
                ...
                this.seatList[0].curDate.length > 0 &&
                this.seatList[0].curDate[0].sitDate.length > 0
              ) {
                this.seatList[0].curDate[0].sitDate.splice(0, 1);
              }
              for (let i = 0; i < this.seatList.length; i++) {
                //如果座位里有时间段可以选择，则设置为可预定
                if (
                  this.seatList[i].curDate[0].sitDate.length > 0 ||
                  this.seatList[i].curDate[1].sitDate.length
                ) {
